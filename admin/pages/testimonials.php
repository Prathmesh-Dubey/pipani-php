<?php
// admin/pages/testimonials.php - Testimonials Management
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($name) || empty($content)) {
        ...
        if ($id > 0) {
            if (updateTestimonial($id, $data)) {
                $message = 'Testimonial updated successfully!';
                logActivity($_SESSION['user_id'], 'update_testimonial', 'Updated testimonial from: ' . $name);
            } else {
            ...
            if (createTestimonial($data)) {
                ...
                logActivity($_SESSION['user_id'], 'create_testimonial', 'Created testimonial from: ' . $name);
            } else {
...
if (isset($_GET['delete'])) {
    ...
    if (deleteTestimonial($id)) {
        ...
        logActivity($_SESSION['user_id'], 'delete_testimonial', 'Deleted testimonial ID: ' . $id);
    } else {
...
<head>
    ...
    <title>Testimonials - <?= siteName() ?></title>
    ...
    <style>
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .stars-display { color: #FBBF24; font-size: 0.9rem; }
        .sidebar-toggle {
            display: none;
            background: none;
            border: none;
...
<div class="sidebar" id="sidebar">
    ...
    <a href="testimonials.php" class="nav-item active"><i class="fas fa-comment"></i> Testimonials</a>
...
<div class="main-content">
    ...
    <div class="content-card">
        <h5><?= $editItem ? 'Edit Testimonial' : 'Add New Testimonial' ?></h5>
        <form method="POST">
            <?php if ($editItem): ?>
            ...
            <div class="row">
                <div class="col-md-4 mb-3">
                    ...
                    <input type="text" name="name" class="form-control" value="<?= $editItem ? htmlspecialchars($editItem['name']) : '' ?>" required>
                </div>
                <div class="col-md-4 mb-3">
                    <label class="form-label">Company</label>
                    <input type="text" name="company" class="form-control" value="<?= $editItem ? htmlspecialchars($editItem['company']) : '' ?>">
                </div>
                <div class="col-md-4 mb-3">
                    <label class="form-label">Position</label>
                    <input type="text" name="position" class="form-control" value="<?= $editItem ? htmlspecialchars($editItem['position']) : '' ?>">
                </div>
            </div>
            <div class="row">
                <div class="col-md-6 mb-3">
                    ...
                    <select name="rating" class="form-select">
                        <?php for ($i = 1; $i <= 5; $i++): ?>
                            <option value="<?= $i ?>" <?= $editItem && $editItem['rating'] == $i ? 'selected' : '' ?>><?= $i ?> Star<?= $i > 1 ? 's' : '' ?></option>
                        <?php endfor; ?>
                    </select>
                </div>
                <div class="col-md-6 mb-3">
            ...
            <div class="mb-3">
                ...
                <textarea name="content" class="form-control" rows="4" required><?= $editItem ? htmlspecialchars($editItem['content']) : '' ?></textarea>
            </div>
                ...
                <tbody>
                    <?php foreach ($testimonials as $item): ?>
                        <tr>
                            ...
                            <td><?= htmlspecialchars(substr($item['content'], 0, 60)) ?>...</td>
                            <td>
                            ...
                            <td>
                                <a href="testimonials.php?edit=<?= $item['id'] ?>" class="btn btn-sm btn-primary">Edit</a>
                                <a href="testimonials.php?delete=<?= $item['id'] ?>" class="btn btn-sm btn-danger" onclick="return confirm('Delete this testimonial?')">Delete</a>
                            </td>
                        </tr>
                    <?php endforeach; ?>
